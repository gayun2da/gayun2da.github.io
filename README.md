# Gayun's Profile

You can use the [editor on GitHub](https://github.com/gayun2da/gayun2da.github.io/edit/main/README.md) to maintain and preview the content for your website in Markdown files.

Whenever you commit to this repository, GitHub Pages will run [Jekyll](https://jekyllrb.com/) to rebuild the pages in your site, from the content in your Markdown files.

![아무이미지나](https://user-images.githubusercontent.com/70933909/119264836-af8a0c00-bc1f-11eb-8ddc-7c892b7e7a7f.png)


# 안녕하세요 방가루 ~ ^.^
김가윤입니다. 💨

저와 함께 마크다운 마스터 되어봅시다 😏 

## 마크다운이란?

2004년, 존 그루버와 에런 스워츠가 만든 마크다운(markdown)은 일반 텍스트 기반의 경량 마크업 언어이며, 일반 마크업 언어에 비해 문법이 쉽고 간단한 것이 특징이라고 합니다.

### 1. 장점 
    - 간결하다
    - 다양한 형태로 변환 가능하다


### 2. 단점
    - 표준이 없다.
    - 모든 HTML 마크업을 대신하지 못한다.

등등이 있겠습니다 :)


##### 이제 마크다운의 문법들에 대해 알아볼 차례입니다!

<br>

## 01 두 문단 사이의 빈 줄
문단과 문단 사이 빈 줄은 엔터키 두 번으로 표현해줍니다.

```
소은아

틱톡 그만 보고 공부하자 🔪
```
<span style="color:blue">결과는 ?</span>
  
소은아

틱톡 그만 보고 공부하자 🔪

##### 이렇게 나온답니다 와아 ~ 신기해라 😊
<br>

## 02 줄바꿈
줄바꿈에는 두 가지 방법이 있습니다 !

#### 1. 문장 뒤에 스페이스바 두 번

```
너무 배고프네요.  
마라탕이 먹고싶다..!
```

너무 배고프네요.  
마라탕이 먹고싶다..!

#### 2. 문장 뒤에 줄바꿈 태그인 `<br>`을 사용

```
너무 배고파요 ㅠ. ㅠ <br> 마라탕이 먹고싶다..!
```

너무 배고파요 ㅠ. ㅠ  <br>  마라탕이 먹고싶다..!

`<br>은 줄바꿈해주는 HTML 태그입니다 !`

<br>

## 03 제목 적는 방법 !

```
# 제목 01
## 제목 02
### 제목 03
#### 제목 04
##### 제목 05
###### 제목 06
```
# 제목 01
## 제목 02
### 제목 03
#### 제목 04
##### 제목 05
###### 제목 06
<br>

```
이런 방법으로도 가능해요
======

진우야 사랑한다 💔
------
```

위 처럼 "=", "-" 두 개로도 표현이 가능 하답니다 :)
======

얍얍얍 
------
  
신기하죠 !! 🤓

<br>

## 04 목록 작성 방법

### - 순서 리스트 작성법
```
1. 첫번째
2. 두번째
3. 세번째
```
전부 다 같은 숫자를 넣어도 제대로 부여됩니다!
```
1. 이소은
1. 멧소은
1. 멧돼지
```
숫자 없는 리스트 작성 방법은?
```
- 강원도 감자
- 군고구마
- 바나나 알러지 원숭이
- 내가 좋아하는 딸기
- 하나더!
```
진짜 간단하죠? 😉 

이번엔 순서 없는 목록 작성 해볼게요 !

### - 숫자 없는 리스트 작성법

* 선호
  * 선호
    * 선호

+ 서노
  + 서노
    + 서노

- 선우
  - 선우
    - 선우

```
* 선호
  * 선호
    * 선호

+ 서노
  + 서노
    + 서노

- 선우
  - 선우
    - 선우
```

짜잔 🤗

이걸 응용해서
+ 진우
  - 진우
    + 진우
      * 진우

```
+ 진우
  - 진우
    + 진우
      * 진우
```

이렇게 섞어서도 사용할 수있겠죠?

그리고 꿀팁으로 ...
멋쟁이 진효가 알려준 ..!
## 05 체크박스 만들기!

+ [ ] 기윤
+ [ ] 가윤

```
+ [ ] 기윤
+ [ ] 가윤
```

네모 칸에 체크 표시 하는 방법은 대괄호 안에 알파벳 x 넣기!

*( (x,X) 대소문자 관계 없이 뭘 넣어도 됩니다!)*

+ [X] 기윤
+ [ ] 가윤

```
+ [X] 기윤
+ [ ] 가윤
```

아쉽게도 마우스로 클릭은 안된다고 해요.. 🙄
<br>

## 06 글씨 강조 하는 법!

#### 1. 볼드체
**첫 번째 방법**
__두 번째 방법__

```
**첫 번째 방법**
__두 번째 방법__
```

#### 2. 이탤릭체
*첫 번째 방법*
_두 번째 방법_

```
*첫 번째 방법*
_두 번째 방법_
```

#### 3. 취소선
~~이소은 바보~~

```
~~이소은 바보~~
```

### 그리고 이건 꿀팁 ! 이탤릭체랑 볼드체를 중복 사용할 수있어요 !!!
**_짜잔~_ 신기하죠!**

```
**_짜잔!_ 신기하죠!**
```

<br>




## 07 코드블럭 쓰는 법! 😀


#### 1. 인라인 코드

```
이것이 `인라인 코드` 입니다 ! 
```

이것이 `인라인 코드` 입니다 ! 


#### 2. 코드 블럭

1. 스페이스바 4번 사용

    #include <iostream>

    using namespace std;

    void main(){
        cout << "HelloWorld" << endl;
    }

2. "~~~" 사용

~~~
#include <iostream>

    using namespace std;

    void main(){
        cout << "HelloWorld" << endl;
    }
~~~

3. "```" 사용


```
#include <iostream>

using namespace std;

void main(){
    cout << "HelloWorld" << endl;
}
```



("```") 백틱 뒤에 언어를 써넣으면 글자에 색상이 들어가게됩니다! 보기 훨씬 쉬워졌네요 ! 😯

```cpp
#include <iostream>

using namespace std;

void main(){
    cout << "HelloWorld" << endl;
}
```  
<br>

## 08 수평선

```
* * *

***

*****

- - -

---------------------------------------

```

결과는 !


* * *

***

*****

- - -

---------------------------------------

<br>

## 09 인용문

> ">" 개수에
>> 따라서 중첩이
>>> 가능합니다 !! 

<br>

## 10 링크 넣기!

`[타이틀](주소)` 이런 식으로 사용하면 됩니다.

```
[네이버](https://www.naver.com/)
```

[네이버](https://www.naver.com/)

짠!

<br>

# 11 이미지 넣는 법

먼저 깃허브 프로필에 들어가주세요

![1](https://user-images.githubusercontent.com/70933909/120929789-65c41a00-c725-11eb-9922-b01c6f997068.png)

Repositories를 찾아서 눌러줍니다

![2](https://user-images.githubusercontent.com/70933909/120929809-7aa0ad80-c725-11eb-9b73-87e66f140838.png)

아무 프로젝트나 들어가줍니다

![3](https://user-images.githubusercontent.com/70933909/120929811-7bd1da80-c725-11eb-86f9-4c6d458229de.png)

issues 들어가 줍시당

![4](https://user-images.githubusercontent.com/70933909/120929812-7c6a7100-c725-11eb-849b-307c1f6e185d.png)

new issue 생성!

![5](https://user-images.githubusercontent.com/70933909/120929814-7d9b9e00-c725-11eb-89e0-22c3c32fd904.png)

코멘트 적는 란에 이미지를 드래그 해줘요

![6](https://user-images.githubusercontent.com/70933909/120929817-7e343480-c725-11eb-96b0-df83c3ba7846.png)

그럼 이렇게 이미지와 이미지 파일 이름이 나오는데 이걸 그대로 코드에 복붙하면 이미지가 업로드 됩니다!

#### 예시

```
![이미지 이름](이미지 파일 이름)

![KakaoTalk_20201222_223057689](https://user-images.githubusercontent.com/70933909/120668339-5779c200-c4c9-11eb-8ed3-1a519df84e3c.png)
```
결과는??

![KakaoTalk_20201222_223057689](https://user-images.githubusercontent.com/70933909/120668339-5779c200-c4c9-11eb-8ed3-1a519df84e3c.png)


잘 나오네요 :)

![아무이미지나](https://user-images.githubusercontent.com/70933909/119264519-85841a00-bc1e-11eb-978b-7fd90893eeec.jpg)


For more details see [GitHub Flavored Markdown](https://guides.github.com/features/mastering-markdown/).

### Jekyll Themes

Your Pages site will use the layout and styles from the Jekyll theme you have selected in your [repository settings](https://github.com/gayun2da/gayun2da.github.io/settings/pages). The name of this theme is saved in the Jekyll `_config.yml` configuration file.

### Support or Contact

Having trouble with Pages? Check out our [documentation](https://docs.github.com/categories/github-pages-basics/) or [contact support](https://support.github.com/contact) and we’ll help you sort it out.
